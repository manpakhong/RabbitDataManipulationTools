...
public class FlowTestJobsProviderExecutorImpl implements FlowTestJobsExecutor<FlowTestJobDto> {
	...
	public FlowTestJobsProviderExecutorImpl(BlockingQueue<FlowTestJobDto> q) {
		init(q);
	}
	...
	private void init(BlockingQueue<FlowTestJobDto> q){
		try{
			...
			SLEEP_TIME = sysProperties.getProducerSleepTime();
	...
	protected void execute(FlowTestJobsProviderThread<FlowTestJobDto> providerThread){
		try{
			executor.execute(providerThread);
		} catch (Exception e){
			logger.error(className + ".execute() - providerThread=" + providerThread, e);
		}
	}
			...
			while (true){
				if (runningEnabled){
					if (queue != null){
						...
						FlowTestJobsProviderThread<FlowTestJobDto> providerThread = new FlowTestJobsProviderThreadImpl(queue, tid.get());
