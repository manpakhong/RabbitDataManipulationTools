...
import com.rabbitforever.datamanipulation.helpers.CaptureVoHelper;
...
public class MoveSelectedRecordDownButtonActionHandler implements ActionListener {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	private String className = this.getClass().getName();
	private MainScreenView mainView;
	private CaptureVoHelper captureVoHelper;
	public MoveSelectedRecordDownButtonActionHandler(MainScreenView mainView){
		...
		try{
			...
			if (selectedCaptureVo != null){
				for (int i = 0; i < captureVoList.size(); i++){
					...
					if (selectedCaptureVo.getTableName() != null && 
							selectedCaptureVo.getOutputXmlFileName() != null){
						if (captureVo.getTableName() != null && captureVo.getOutputXmlFileName() != null){
							if (selectedCaptureVo.getTableName().equals(captureVo.getTableName()) &&
									selectedCaptureVo.getOutputXmlFileName().equals(captureVo.getOutputXmlFileName())){
								if(i != captureVoList.size() -1){
									positionA = i;
